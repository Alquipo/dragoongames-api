...
const Register = ({
  ...
  const terms = (
    <FormattedMessage id="Auth.privacy-policy-agreement.terms" key="1">
      {(content) => (
        <Span onClick={(e) => handleClick(e, "terms")}>{content}</Span>
      )}
    </FormattedMessage>
  );
  const policy = (
    <FormattedMessage id="Auth.privacy-policy-agreement.policy" key="2">
      {(content) => (
        <Span onClick={(e) => handleClick(e, "privacy")}>{content}</Span>
      )}
    </FormattedMessage>
  );
          ...
          <Box errorMessage={get(requestError, "errorMessage", null)}>
            <form onSubmit={onSubmit}>
              ...
              <Padded top size="md">
                <Button
                  type="submit"
                  style={{ background: "#CD113B", borderColor: "#CD113B" }}
      ...
      {!noSignin && (
        <AuthLink label="Auth.link.signin" to="/auth/login">
          <Text fontSize="md">
            ...
            &nbsp;
            <Text fontSize="md" color="#CD113B" as="span">
